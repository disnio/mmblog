...
export async function createArticle(req, res) {
  ...
  let nares = na.save().catch(function(err) {
    res.status(500, err)
  });
...
export async function getArticles (req, res) {
  ...
  let pageNum;
  let aCount;
  let publishObj = publish ? {publish: 1} : {publish: 0};
  if(page !== 0){
    skip = limit*(page-1)
  }
  if(tag == ""){
    ...
    aArr = await model.find(queryObj).populate("tags").sort({createdAt:-1}).limit(limit)
      .skip(skip).catch(err=>{
        res.status(500, err)
      });
  ...
  pageNum = Math.ceil(aCount/limit);
  res.json({
    success:true,
    articleArr: aArr,
    allPage: pageNum
